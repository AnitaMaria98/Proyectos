...
public class BDController {
		...
		try {
			...
			while (rs.next() == true) {
				jugadores.add(new Jugador(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getString(4), rs.getInt(5),
						rs.getString(6), (Carta) rs.getObject(7)));
			}
			miStatement.close();
			rs.close();
	...
	public Carta dameCartaSimple(int cod_jugador) {
		...
		try {
			...
			System.out.println("Error en dameCartaSimple del BDController" + e.getMessage());
		}
		return carta;	
	}
	public ArrayList<Carta> CartaSimple(int cod_jugador,String nombre) {
		ArrayList<Carta> cartas=new ArrayList<Carta>();
		try {
			Statement miStatement=this.miConexion.createStatement();
			ResultSet rs=miStatement.executeQuery("SELECT * FROM cartas WHERE cod_jugador='"+cod_jugador+"' AND nombre='"+nombre+"'");
			if (rs.first() == true) {
				Carta carta=new Carta(rs.getInt(3), rs.getString(4), rs.getInt(5), rs.getInt(6), rs.getInt(7), rs.getInt(8), rs.getInt(9), rs.getInt(10), rs.getInt(11), rs.getInt(12), rs.getInt(13));
			    cartas.add(carta);
			}
			miStatement.close();
			rs.close();
			...
			// TODO Auto-generated catch block
			System.out.println("Error en CartaSimple del BDController" + e.getMessage());
		}
		return cartas;
	}
